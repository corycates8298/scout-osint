...
def get_jobs(target, full=False, stealth=False, venv_cmd=None):
    ...
    jobs = {
        "WHOIS": venv_cmd(f"python3 -c \"import whois; import json; w=whois.whois('{safe_py}'); print(json.dumps(dict(w), indent=2, default=str))\" 2>/dev/null"),
        ...
        "Wayback Machine": venv_cmd(f"python3 -c \"from waybackpy import WaybackMachineCDXServerAPI; cdx=WaybackMachineCDXServerAPI('{safe_py}'); [print(s.archive_url) for i,s in enumerate(cdx.snapshots()) if i<15]\" 2>/dev/null"),
    }
    ...
    if full:
        ...
        jobs["BuiltWith"] = venv_cmd(f"python3 -c \"import builtwith; import json; r=builtwith.parse('https://{safe_py}'); print(json.dumps(r, indent=2))\" 2>/dev/null")
